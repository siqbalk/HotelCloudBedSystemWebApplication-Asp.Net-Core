        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (count == 0)
                    {
                        var result1 = await _userManager.AddToRoleAsync(user, "User").ConfigureAwait(true);
                        var UserByEmail = _userManager.FindByEmailAsync(Input.Email).Result;
                        var rolebyname = _roleManager.FindByNameAsync("User").Result;
                        if (UserByEmail != null)
                        {
                            UserByEmail.AppRole = rolebyname;
